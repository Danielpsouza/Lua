...
function love.load()
	...
	batedor = {x = xTela/2, y = 587, velocidade = 200}
	--"Bola"
	partidaBola = true
	tempoBolaInicial = 3
...
function love.draw()
	...
	--Desenho da barreira, posição, ângulo e tamanho nos eixos, ponto pivô nos eixos
...
function love.update(dt)
	...
	tempoBolaAtual = tempoBolaAtual - (10*dt)
	...
	function gol()
		for i, alvo in ipairs(bolas) do
			if verifGol(alvo.xPos, alvo.yPos, alvo.wPos, alvo.hPos, bolas.xPosicao, bolas.yPosicao, imgBola:getWidth()/2, imgBola:getHeight()/2) then
				table.remove(bolas,i)
				pontos = pontos+1
			end
		end
